...
for "dynamic routes" (the pages that have parameters within their URLs): **if an
URL is of the form `/posts/[postSlug]`, what are all the possible valid values
that `[postSlug]` can have?** Without knowing this, you SSG cannot know what
pages to generate. This is the question that `getStaticPaths` exists to answer.
