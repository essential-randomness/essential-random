...
each page as it goes to create it, similarly to how we manually pass props to
components.
...
Note that **unlike params which the SSG _must have_ to work, props is completely
  ...
  return blogEntries.map((blogPost) => {
    return {
      ...
      props: {
        ...
        // into the renderContent property, like we do with every other variable.
...
<details>
  <summary>Optional: Open for an advanced trick! (pre-rendering the component within `getStaticPaths`)</summary>
  <div>
...
// Now Component is an AstroComponent, rather than the rendering function
type PageProperties = {
...
```
    ...
    If we want to pre-render, we can fix this in a somewhat-simple way: by waiting for
...
export async function getStaticPaths() {
    const blogEntries = await getCollection("posts");
    // We Aell astro to wait for all promises in the array to be resolved before
    // reading the value.
    return Promise.all(
        ...
        return {
            ...
            props: {
                ...
                // it will need to be awaited, which is what forced us to use Promise.all.
...
```
    ...
    We can then render the component in our `.astro` files by doing:
