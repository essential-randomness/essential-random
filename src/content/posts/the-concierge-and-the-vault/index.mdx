---
title: "Communities on AT: the Concierge, the Vault, and the Fuck-ass Astro Website"
...
Community, since Boris ~~torpedoed my week~~ wisely suggested I say something on
the topic of ATproto Communities.
...
an ATproto community](https://atmosphere.community), and spent a few years
looking at "how the fuck do we community"—**finally getting off my ass and
writing down what I know, believe, and _have been taught_ about how we should do
community on ATproto.**
...
its members; a community is not its admins, moderators, or (for what is worth)
...
moment has come for us, folks are racking their brains _(together!)_ so
...
Last week, Zicklag generously lent me some time to walk through the Arbiter's
design together. My plan was to learn its more practical ins and outs, so I
could jump on stream myself and walk less-technical "AT-izens" through the shape
of ATproto communities—something now postponed (not replaced) by this article.
...
maybe it's something different. Judgement is still open—all I
ask right now is that you picture it as Charon from John Wick.
...
Why not, let's call it a Community App! [Habitat](TODO) already has a notion of
installable apps that act on a community's behalf; so does
[OpenSocial](https://opensocial.community/), which I used to prototype
[atmosphere.community](TODO).
...
best teased apart by looking at my experience with OpenSocial[^6], how
today's zero-generation ATproto-based Community Apps interface with
random websites like the [atmosphere.community](TODO), and most importantly
...
distinction between a "User" logging in on a service, and a "Community Member"
logging in on one:
...
- When a "Community Member" logs into a _service_ using their ATmosphere
  ...
  members, or removing _themselves_ for it. After all, they don't have that
...
To get around this limitation _(and stop me if you heard this before)_, **there
...
Let's poke and prod: what happens a Community Member logs into another service
...
This is exactly how [atmosphere.community](TODO) works:
...
<figcaption>
  The design of [atmosphere.community](TODO), our first (spoiler warning)
  *fuck-ass Astro website*
</figcaption>
...
whether anyone cares enough about them that they'll pester a Community Admin to.
...
> piggying on its credentials"! Who cares if that App is the Concierge?
...
**The difference is the third-party Service and the Concierge don't have to
...
Apps_ exist: they shouldn't just be part of this ecosystem, they _will
need to be_. The issue is that, by making the barrier to offering smaller,
complementary, additional services to Community Members _that much higher_,
**_Blessed Apps_'s lack of scalability creates the risk of a "plausibly-deniable
...
"person"—and ask their Users' legitimate requests be fulfilled by someone that
...
- **For fully-public data that can be accessed by everyone,** a Community data
...
_AH!_ As if I'd keep it _that short_[^15]!
...
should _freely_ access at all—not the Admin, not the most trusted Mod, not
_anyone_.** This is all data that belongs to a community, but that in many
Communities today _almost no one (if anyone at all)_ can read or write directly
and in its entirety—nor, for the most part, wants to.
...
...and many more. This category of data often varies between community and
community. Sometimes even across the same one at different ages and stages
of growth.

In the current ATproto world, similar data often ends up stored within
individual App(View)s. And while Permissioned Spaces will "bring some back into
the light", it won't fully eliminate the need for it—for every piece of data
you bring, I can think of a community that may want to get it private.

In the future AT-Communities world, unless a solution for Fully Private Data is
found, it will end up stored within individual Arbiters (or Concierges) instead.

### Enter the Vault

I call this place "the Vault": a community-owned store of records whose shape is
defined by Lexicons, with actions taken via a specific set of XRPC requests, not
meant to be directly accessible to members—a back*room* of sensitive or private
_community_ data, locked behind a counter, with a "professional" in charge of
retrieving and storing information on a need-to-know basis.
...
Instead of free access, granular permissions are assigned (even at the level of
individual records), and data is fetched or written—once again!—on behalf of
the entity making the request. Who may grant that access is itself a
high-clearance, granular decision.

In a way, you can think of "the Vault" as the community's own internal SQLite
database, like the one many AppViews build: just a standardized one with no
access rights directly granted. ATproto applications read and write its content
through the same affordances we already use—Lexicons and XRPC—which keeps its
content interoperable across delegated services. And—even better!—we can also
leverage other affordances that already exist in the protocol to keep the
vault's _content_ movable across hosts: simply changing the `#vault` service
endpoint a community DID document points to, just like one would with a PDS.

> **_Necessary Disclaimer_:** yes, whoever holds the raw infrastructure can read
> anything in the Vault. And someone _must_ be able to, whether that's the
> Vault's owner or whoever's charged with auditing it. That's not the threat the
> Vault defends against: the Vault hides data from _everyone else_ (the mod, the
> admin, the volunteer...) whose access should come from doing a _job_, not from
> holding a _key_.

There's a _load-bearing part_ here worth repeating: the data in the Vault is
not owned by the _Concierge_, nor by whoever mediates access to it. It's a
place _owned by a DID_, the same way a PDS is owned by the community and
surfaced as a specific data-storing service. Data that AppViews currently hold
internally can either be mirrored into the Vault or placed there to
start—either way, the data travels with the DID, not with whoever's running the
AppView at the moment.

In short: the Vault is _where the off-protocol state goes so the AppView
doesn't have to hold it_—an on-protocol way to keep off-network (but still
shareable!) data associated with a specific DID. A data store that "speaks"
ATproto-language, but is _not_ necessarily ATproto-shaped. Not today, at
least.
...
## _Sleight of Hand:_ It was Everyone All Along!

TODO

### Issue/Solution 1: (Anonymous) AskBox Vault

TODO

### Issue/Solution 2: AskBox Concierge

TODO

## It's Not Over Yet (but I swear it'll be soon): Enter Notifications

TODO

### Let People ~~Enjoy~~ Solve Things

TODO

## Epilogue: App-Centric Networks, User-Centric Networks
...
[^1]:
    **P**ersonal **D**ata **S**erver: your personal folder of data in the
    ...
    your Bluesky posts is stored inside this. If you made your account through
...
[^9]: TODO: Something something other PDS implementation.
...
[^13]: ((Studio Audience Laugh-track plays)) <small>...sorry.</small>

[^14]: ![](./it-depends.png)

[^15]: ((Studio Audience Laugh-track plays again)) <small>...i'm so sorry.</small>
